...
class AgentDecisionGraphTests(unittest.TestCase):
    ...
    def test_safety_gate_does_not_require_legacy_design_review(self) -> None:
        panel = PanelRequest(
        ...
        self.assertTrue(update["execution_authorized"])
        self.assertEqual(update["authorization_reason"], "EXECUTION_PREFLIGHT_PASSED")
    ...
    def test_same_reference_and_boundary_no_longer_runs_mock_review(self) -> None:
        with (
            patch.object(type(graph_module.llm), "invoke", side_effect=[
                action_output("FR100"),
        ...
        self.assertNotIn("design_review", state)
        self.assertTrue(state["execution_authorized"])
        create.assert_called_once()
