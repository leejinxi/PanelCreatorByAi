...
- 只有有效 `PanelRequest`、所有工程对象唯一匹配、创建前评审无阻断项且最终决策允许时，Safety Gate 才授权 CAD Mock。
- CLI 与浏览器共用同一 Graph；Web 以七段行为链展示两次 Qwen 调用、首次决策、Mock 工程查询、板架智能评审、安全门禁和 CAD 是否调用，并分别显示参数解析与结果评估耗时。
- Demo 评审会对比显式配置的邻近 Mock 板架、展示计划调整和决策护照；CCS、结构强度及真实 CAD 几何固定标记为“未验证”。
