...
```text
在 FR100 创建14mm厚AH36板架，边界 >SL10 <LV5
```
...
本方案的批次 A–D 已完成：新增强类型 `AgentDecision` 与工程查询 DTO、精选 Demo 工程目录、确定性对象匹配、两步受控决策链、fallback、safety override、Safety Gate，以及 Web 决策时间线和对象逐项状态。用户原文中的 Demo 别名和目录外 FR/SL/LV 查询词会被确定性保留，避免模型漏抽取导致演示退化。

自动化回归共 184 项，其中 183 项通过，1 项真实 Ollama E2E 按设计默认跳过。真实本地 Qwen 页面验收覆盖：FR100 成功并授权模拟创建、主甲板歧义后请求选择、FR999 未找到后阻止 CAD。页面和响应始终标注 Mock Project Context 与模拟 CAD；未引入真实 CAD、RAG、多 Agent 或其他结构类型。
