...
|---|---|---:|---|
| A | 板架边界数据升级未完整迁移 | 1 个板架及 14 个子构件 | 唯一恢复对象，确认后更新 |
| B | 2 个父板架需要重新计算 | 2 个板架及 9 个子构件 | 低风险批量重算 |
| C | 肘板边界在关联更新后失效 | 6 个肘板 | 人工重选边界并预览形体 |
...
### 8.0 Mock 输入与 Agent 输出边界

为避免演示页面只是回显预置结论，`mock_data/demo_model_errors.json` 必须模拟 CAD Provider 的原始返回，而不是 Agent 的分析报告。

Mock 输入只允许包含：

- 工程版本和边界数据升级事件；
- 未分组的错误对象、对象类型、错误码和父对象 ID；
- CAD 诊断得到的可执行操作、预校验状态、候选对象 ID 和是否改变设计意图。
...
- 问题组名称和归类结论；
- `route`；
- 聚合后的 `RepairCandidate` 和 `operation_id`。

Agent 分析阶段负责沿错误对象的父子关系定位主要错误对象，将子构件错误关联到上游问题，依据错误码规则合并同类问题，再结合 CAD 诊断候选生成风险路线与更新方案。当前 Demo 使用确定性策略完成这一过程，保证结果可复现、可解释、可测试；LLM 不得猜测工程对象关系或 CAD 事实。
...
在执行分析前提供“了解 Mock 场景”入口，先向观众交代演示假设：板架边界数据由0.1-poc四向字段升级为0.2-poc边界列表，货船详细设计模型随之执行批量迁移和关联重算；全船约8,000个板架、约120,000个板架下子构件，当前36个错误只是从固定 Mock 数据中选择的演示快照。数量级只用于说明人工治理压力，不代表特定真实船型或实时 CAD 工程统计。
...
板架新建或更新操作提供“查看板架更新方案”入口，不在错误治理页重复堆叠板架参数和 MCP 报文。
...
- “查看板架更新方案”携带正确 `taskId`、`errorGroupId` 和 `operationId`。
