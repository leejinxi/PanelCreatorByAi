...
`GROUP-C` 模拟板架关联更新后的肘板形体错误：3个肘板因所依赖板架的正方向变化而边界失效，另外3个因骨材安装面变化而失效。CAD不能安全沿用原边界，Agent只负责集中分流，必须由设计人员重选边界并预览形体。

错误分析前可打开“了解 Mock 场景”：页面假设板架边界数据由 `0.1-poc` 四向字段升级为 `0.2-poc` 边界列表，货船详细设计模型随之从 `DEMO-REV-17` 升级到 `DEMO-REV-18`；以约8,000个板架、约120,000个板架下子构件说明全船人工排查的数量级，并明确36个错误只是精选 Mock 快照、不是实时 CAD 查询，也不代表特定真实船型。

错误组 `GROUP-A` 改为“板架边界数据升级未完整迁移”：FR108仍有1条旧边界没有写入新版列表，CAD返回唯一可恢复对象 `LONG-BHD-02`。其“查看板架更新方案”只读深链通过 `OP-REPAIR-108-01` 展示迁移批次、源/目标边界Schema、目标板架、恢复对象、工程版本、决策依据和安全检查；详情视图不会重新调用 CAD。对应 API 为 `/api/model-errors/analyze`、`/api/model-errors/repair/{task_id}`、`/api/model-errors/status/{task_id}` 和 `/api/operations/{operation_id}`。
...
浏览器若仍保留旧五节点页面，而服务端已返回新版六节点 Trace，旧 DOM 无法匹配节点名，曾导致请求结束后残留“正在调用/等待请求”。当前静态资源已升级为 `error-governance-v9`；创建页只显示六个实际节点，旧评审节点和决策护照不再参与结果展示。错误快照与运行中Schema不兼容时返回可操作的503说明，不再只显示通用500；代码或Schema更新后仍应重启旧服务进程并执行一次 `Ctrl+F5`。
