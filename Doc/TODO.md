...
已提交代码基线：`f9792bf` — 增加全船结构模型错误治理演示主流程。

当前工作区正在开发“全船模型错误治理 Agent”，按用户要求暂不提交、不推送。后续开发依据见 [全船模型错误治理 Agent 开发方案 v2.0](Plan/AI_Ship_CAD_Copilot_全船模型错误治理Agent开发方案_v2.0.md)；v1.0 保留用于追溯初始设计。

## P0：全船模型错误治理 Demo

- [x] 将“模型错误治理”设为唯一主入口，移除顶部业务 Tab；板架创建作为自动更新说明和单对象决策详情的下钻能力保留。
- [x] 在自动重算组增加“板架自动更新说明”，展示错误对象和更新编排，并在下钻页复用现有板架创建/决策链。
- [x] 删除重复的“板架决策护照”页面卡片，保留结构化决策、Safety Gate、执行Trace和只读操作审计。
- [x] 新增强类型错误对象、根因组、修复候选、汇总、治理报告和只读板架操作记录 Schema。
- [x] 新增36个错误节点、4个根因组的显式 Mock CAD 错误快照；明确其不代表实时 CAD 工程。
- [x] 在分析前增加可展开的 Mock 场景说明，展示版本演进假设、约8,000个板架和约120,000个子构件的演示数量级及能力边界。
- [x] 将低频“外壳对象替换”场景改为板架边界0.1-poc到0.2-poc数据升级，展示部分板架迁移不完整、确认后更新和关联重算。
- [x] 将“开孔拓扑候选”改为肘板边界失效，覆盖板架正方向变化和骨材安装面变化两类依赖问题。
- [x] 错误快照与运行中Schema不兼容时返回受控503和重启提示，前端展示服务端可操作错误，不再退化为通用500。
- [x] 移除 Mock JSON 中预置的 `GROUP-A~D`、根对象标记、问题结论和处置路线；改为输入36条原始错误、父子关系和CAD诊断候选，由Agent确定性分析生成4个问题组与分流决策。
- [x] 将固定路线拆为每组 `allowed_routes`，新增独立错误治理 LangGraph，由Qwen结合用户策略动态选择，越权或异常时使用fallback / safety override。
- [x] Web增加“只分析 / 安全项自动处理 / 所有修改需确认”三种策略，展示允许路线、Agent选择、决策来源和七步行为轨迹。
- [x] 修复接口接收显式确认组，逐组返回Safety Gate授权结果；只分析模式不得模拟恢复对象。
- [x] 展示根因/级联识别和自动、确认后、人工、Provider异常四类分流。
- [x] 增加五步治理轨迹和结果收口；模拟恢复26个错误，保留10个待人工/研发处理对象。
- [x] 增加 `operationId` 只读详情 API 与页面深链；查看详情不重新触发 CAD 调用。
- [x] 将创建前智能评审从默认 Graph、Safety Gate 和主页面降级；兼容代码暂不删除。
- [x] 增加错误治理工具/API/板架只读操作记录测试，并保持既有创建链路回归。
- [ ] 在1366x768与1920x1080完成主入口、自动更新说明和决策详情最终视觉验收并冻结演示截图。
- [ ] 获得真实 CAD 接口资料后，定义错误快照、依赖关系、修复预览、批量执行和复核 MCP 契约。
